    ...
    (function ($, undefined) {
        ...
        ui.Select = List.extend({
            ...
            _parentWidget: function () {
            	...
            	// We want to have other widgets in cascade so use the 'handler' key to
            	// access any widget bound to the element
                if (!parent) {
                	var parentElement = $('#' + this.options.cascadeFrom);
                	parent = parentElement.data("handler");
                	if (!parent || !(parent instanceof kendo.ui.List))
                		return;
                }
        ...
        function getSelectedOption(select) {
        	...
        	// start of the patch by Corrigo (single-part)
        	//
        	// original accessor doesn't return value of selected element if it
        	// set through options when datasource hasn't loaded any data yet
        	//
        	// return index > -1 ? select.options[index] : {}; // original code
