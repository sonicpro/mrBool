...
(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        var Grid = kendo.ui.DataBoundWidget.extend({
        	...
        	///////////////////////////////////////////////////////////////////
        	// start of the patch by Corrigo (single-part)
        	// this function is patched more or less fully with custom code
        	// We change logic that actually handles transformation of mouse
        	// movement into size of columns. One of important parts is that
        	// we start using percents instead of px
        	// main pieces of patched logic are inside inner "resize" method
        	// additional code in other places mostly prepares variables for that code
            _resizable: function () {
            	...
            	if (options.resizable) {
            		...
            		that.resizable = new ui.Resizable(container.add(that.lockedHeader), {
            			...
            			start: function (e) {
            				...
            				colIndex = index;
            				cols = that.tbody.parent().find("col");
            				origColPerc = [];
            				th.parent().children(":visible").each(function (i, innerTh) {
            					origColPerc.push($(innerTh).outerWidth() / gridWidth);
            				});
            				minColPerc = Math.min(2.0 / 3.0 / origColPerc.length, that.minColWidth || 0.05);
            				maxColPerc = Math.min(1 - minColPerc * (origColPerc.length - 1), 0.9);
            			},
            			resize: function (e) {
            				...
            				if (options.scrollable) {
            					...
            					//todo: fix scrollable case
            					// that case is harder because grid is dranw using sevearl tables
            					// and we need to updated all of them. Currently as we don't use
            					// scrollable grids we just leave original implementation
            					var curOrigPerc = origColPerc[colIndex],
									colsLen = cols.length,
									newCurPerc = Math.min(Math.max(curOrigPerc * currentWidth / columnWidth, minColPerc), maxColPerc);
            					if ((newCurPerc < minColPerc) || (newCurPerc > maxColPerc))
            						return;

            					var fixedCols = [];
            					var newColPerc = new Array(colsLen);
            					// enum
            					var RESIZABLE = 0, MIN = 1, MAX = 2, CURRENT = 3;
            					for (i = 0; i < colsLen; i++)
            						fixedCols.push(RESIZABLE);
            					fixedCols[colIndex] = CURRENT;

            					// This method tries to re-distribute width (percents) to meet newCurPerc AND
            					// restriction on max/min column width. For columns other than current
            					// and the ones that have reached min/max limit we want to scale them
            					// proportionaly to each other.
            					// Generally logic is following:
            					// 1) first process columns that are already known to hit some fixed width
            					//    limit (min, max or the current column)
            					// 2) Calculate total percent fixed columns take now and total percent they took originally
            					// 3) Distribute the rest of percent proportionally between columns that can vary in size
            					//    If during this process we find another column that hits limit, remember it
            					//    but continue current attempt to find all such columns
            					// 4) Return to the caller information whether at step #3 we find new "bad" column
            					//    Basing on this information caller might want to call tryDistributeWidth again
            					function tryDistributeWidth() {
            						var i;
            						var nonFixedCount = 0,
										fixedCount = 0,
										distributedOirgPerc = 0,
										distributedPerc = 0,
										newPerc;
            						var success = true;
            						for (i = 0; i < colsLen; i++) {
            							...
            							if (RESIZABLE === fc) {
            								...
            								fixedCount++;
            								distributedOirgPerc += origColPerc[i];
            								var fixedWidth;
            								if (MIN === fc)
            									fixedWidth = minColPerc;
            								else if (MAX === fc)
            									fixedWidth = maxColPerc;
            								else if (CURRENT === fc)
            									fixedWidth = newCurPerc;
            								else
            									throw new Error("Unexpected enum value '" + fc + "'");
            								newColPerc[i] = fixedWidth;
            								distributedPerc += fixedWidth;
            							}
            						}
            						var total = 1 - distributedPerc,
										otherK = (nonFixedCount > 0) ? (total / (1 - distributedOirgPerc)) : 0;
            						// don't stop cycle on first failure
            						// find all columns that doesn't fit in min/max range
            						// under current fixed columns configuration
            						for (i = 0; i < colsLen; i++) {
            							var fc = fixedCols[i];
            							// for i === colIndex, fc === CURRENT and thus next if is always true
            							if (RESIZABLE !== fc)
            								continue;
            							newColPerc[i] = newPerc = origColPerc[i] * otherK;
            							if (newPerc < minColPerc) {
            								fixedCols[i] = MIN;
            								newColPerc[i] = minColPerc;
            								success = false;
            							}
            							else if (newPerc > maxColPerc) {
            								fixedCols[i] = MAX;
            								newColPerc[i] = maxColPerc;
            								success = false;
            							}
            						}
            					...
            					// Theoretically if we run tryDistributeWidth long enough
            					// it will find a perfect distribution that meets min/max requirements
            					// However it might take too long. So run either until we find
            					// a perfect distribution or until we reach 3 attempts
            					// It is expected that after 3 attempts distribution should be good enough
            					for (var i = 0; i < 3; i++) {
            						if (tryDistributeWidth())
            							break;
            					}

            					cols.each(function (i, innerCol) {
            						$(innerCol).css('width', newColPerc[i] * 100 + "%");
            					});
            				}
            			},
            			resizeend: function () {
